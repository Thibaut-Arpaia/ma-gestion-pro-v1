...
export function validate(d){
 ...
 ids.add(r.id);if(r.ref!==null){if(!integer(r.ref)||r.ref<1||refs.has(r.ref)||r.ref>=d.next||(r.historical?r.ref>=d.setup.first:r.ref<d.setup.first))throw Error('Numérotation incohérente.');refs.add(r.ref);}else if(!r.historical)throw Error('Référence manquante.');
 }return d;
}
export function change(source,action,p){const d=structuredClone(validate(source));
 ...
 else{const ref=v.historical?(p.ref?Number(p.ref):null):d.next++;const id=d.expenses.reduce((m,r)=>Math.max(m,r.id),0)+1;d.expenses.push({...v,ref,id,cancelled:false});}
 }else if(action==='remove'){const r=d.expenses.find(r=>r.id===p&&!r.cancelled);if(!r)throw Error('Dépense introuvable.');r.cancelled=true;}
 else throw Error('Action inconnue.');d.revision++;return validate(d);
}
